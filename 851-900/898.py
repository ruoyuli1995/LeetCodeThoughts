...
'''
---Thought---
So sad Q_Q. Cannot solve two adjacent problems.
When seeing A.length is no more than 50k, I know it should be solved within O(NlogN), so I try to use sort.
Then I want to solve it with prefix, just like what I do when solving max-sum, but I fail.
The answer is like a DP solution, every step we need only think about one step before. It is also like a prefix solution, but we sholud use a set to record current state.
Actually I think the solution is a little "brute force" hahaha.
'''
